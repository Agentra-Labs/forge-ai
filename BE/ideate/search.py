"""Web search tools for the ideate pipeline.

Provides Serper/Exa-backed search with intent routing, budget limits,
and SearchTrace for source attribution in reports.
"""
...
FRESHNESS_HINTS = (
    "latest", "recent", "today", "current", "currently",
    "trend", "trends", "news", "funding", "raised",
    "pricing", "price", "market", "regulation", "regulatory",
    "incumbent", "competitor", "competitors", "company", "companies",
    "startup", "startups", "buyer", "buyers", "customer", "customers",
    "launch", "launched", "2024", "2025", "2026",
)
...
def choose_providers(intent: SearchIntent) -> list[SearchProvider]:
    ...
    if mode in {"serper", "exa"}:
        preferred = mode
        fallbacks = [p for p in configured if p != preferred]
    ...
    ordered = [p for p in preferred_order if p in configured]
    ...
    *,
    ...
    async with httpx.AsyncClient(timeout=timeout) as client:
        for provider in providers:
            try:
                if provider == "serper":
                    results = await _search_serper(client, query, num_results=_get_result_limit())
                else:
                    results = await _search_exa(client, query, num_results=_get_result_limit())
            ...
            if results:
                response = SearchResponse(
                    query=query, intent=intent, provider=provider,
    ...
    response = SearchResponse(
        query=query, intent=intent, provider=providers[0],
...
def make_web_search_tool(
    *,
    ...
    """Create a budget-limited async web_search callable for use in agent scope."""
    calls_used = 0
...
def make_disabled_web_search_tool(message: str | None = None):
    """Create a disabled web_search stub that returns a static message."""
    disabled_message = (
        message
    ...
    async def web_search(_: str) -> str:
        """Search the web (disabled for this agent).

        Args:
            _: Query string (ignored).
