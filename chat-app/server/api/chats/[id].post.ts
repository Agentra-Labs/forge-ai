    ...
    const stream = createUIMessageStream({
        execute: async ({ writer }) => {
            const result = streamText({
                model,
                system: `${researchModeInstruction} ${viewer.username ? `The user's name is ${viewer.username}.` : ''} Your goal is to provide clear, accurate, and well-structured responses grounded in research evidence.
